...
from wutils.np import get_group_ixs, nan_allclose
from wutils.io import read_pickle, write_pickle
...
def test_ddf_repr():
    ...
    #df = DDF({'shitcol': [u'', u'  ', u'+ ', u'\xa7', u'x ', u'xx', u'd ', u'\xa7\xa7', u'p ', u'? ']})
...
def test_fillna_not_inplace(df):
    ...
    assert nan_allclose(df[0], original_df[0])
...
def test_can_describe_numeric_data(df):
    df.describe()


def test_can_describe_nonnumeric_data(df):
    df['col'] = np.repeat('A', len(df))
    df.describe()
...
def test_appending_not_inplace(df):
    ...
    assert df.is_equal_to(original_df)
...
def test_boolean_array_slicing(df):
    ...
    assert sliced.is_equal_to(df[index])


def test_direct_slicing(df):
    assert df[:5].is_equal_to(df.head())
    assert df[-5:].is_equal_to(df.tail())
    reverse_ix = np.arange(len(df))[::-1]
    assert df[::-1].is_equal_to(df.rowslice(reverse_ix))
...
def test_can_merge_two_ddfs():
    ...
    assert all(ss.np.nan_equality(d3['col3'], np.array([0, 1, np.nan, np.nan])))
...
def test_group_reduce(df, pandas_df):

    def mean_of_first_two_columns(df):
        return np.mean(df[[0, 1]], axis=0)
    pdf = pandas_df.groupby('group_id').apply(mean_of_first_two_columns)
    ddf = df.group_reduce('group_id', mean_of_first_two_columns)
    np.testing.assert_allclose(pdf.values.T, ddf.values)


def test_group_apply_return_df():
    data = {'runner_id': np.array([0]*5+[1]*5), 'x': np.arange(10)}
    df = DDF(data)
    willy_df = df.group_filter(groupby='runner_id', func=slice(0, 2))
    expected_data = {'runner_id': np.array([0]*2 + [1]*2), 'x': np.array([0, 1, 5, 6])}
    expected_df = DDF(expected_data)
    assert willy_df.is_equal_to(expected_df)

    other_willy_df = df.group_filter(groupby='runner_id', func=lambda x: x['x'] > 6)
    expected_data = {'runner_id': np.array([1]*3), 'x': np.array([7, 8, 9])}
    expected_df = DDF(expected_data)
    assert other_willy_df.is_equal_to(expected_df)


def test_group_apply(df, pandas_df):

    def center(df):
        return df[[0]] - np.mean(df[[0]])
    pdf = pandas_df.groupby('group_id').apply(center)
    centered = df.group_apply('group_id', center)
    np.testing.assert_allclose(pdf.values, centered)


def test_caching_group_ixs(df):
    baseline_group_ixs = df.get_group_ixs('group_id')
    repeated_group_ixs = df.get_group_ixs('group_id')
    assert isinstance(baseline_group_ixs, dict)
    assert baseline_group_ixs is repeated_group_ixs


def test_recalculating_group_ixs(df):
    baseline_group_ixs = df.copy().get_group_ixs('group_id')
    ddf_copy = df.copy().append(df, axis=0)
    recalculated_group_ixs = ddf_copy.get_group_ixs('group_id')
    assert baseline_group_ixs is not recalculated_group_ixs
    standard_ixs = ss.slicing.get_group_ixs(ddf_copy['group_id'])
    for k in set(standard_ixs.keys() + recalculated_group_ixs.keys()):
        assert (standard_ixs[k] == recalculated_group_ixs[k]).all()


def test_recalculating_shuffled_ids(df):
    df.get_group_ixs('group_id')
    df['group_id'] = np.random.permutation(df['group_id'])
    new_ixs = ss.slicing.get_group_ixs(df['group_id'])
    expected_ixs = df.copy().get_group_ixs('group_id')
    for k in set(new_ixs.keys() + expected_ixs.keys()):
        assert (new_ixs[k] == expected_ixs[k]).all()


def test_changes_to_ids(df):
    before_ixs = df.get_group_ixs('group_id')
    df['group_id'][0] = -999
    after_ixs = df.get_group_ixs('group_id')
    assert before_ixs is not after_ixs
    expected_ixs = ss.slicing.get_group_ixs(df['group_id'])
    for k in set(after_ixs.keys() + expected_ixs.keys()):
        assert (after_ixs[k] == expected_ixs[k]).all()
...
def test_is_equal_to_returns_false_when_columns_are_not_the_same():
    df = DDF({'col1': np.array([1, 2])})
    df2 = DDF({'col2': np.array([1, 2])})
    assert not df.is_equal_to(df2)


def test_is_equal_to_invariant_to_column_order():
    df = OrderedDict([
            ('col1', np.array([1, 2])),
            ('col2', np.array([2, 1])),
            ])
...
def test_get_cols(df):
    cols = df.get_cols(0)
    assert len(cols) == 1

    df['NewCol'] = df[0]
    cols = df.get_cols('newcol')
    assert len(cols) == 1

    df[b'unicode_col'] = df[0]
    cols = df.get_cols('icode')
    assert len(cols) == 1
...
def test_settings(tmpdir):
    df = DDF()
    style = df.get_option('repr.style')
    if style == 'horizontal':
        new_style = 'vertical'
    elif style == 'vertical':
        new_style = 'horizontal'
    df.set_option('repr.style', new_style)
    updated_style = df.get_option('repr.style')
    assert updated_style == new_style

    # making sure updated settings are not saved
    path = str(tmpdir) + 'df.pkl'
    write_pickle(df, path)
    loaded = read_pickle(path)
    assert loaded.get_option('repr.style') != style
