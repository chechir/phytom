...
class CoreDF(object):
    ...
    def fillna(self, value, inplace=False):
        ...
        if inplace:
            ...
            cols_to_copy = [k for k, v in self._data.items() if any(pd.isnull(v))]
    ...
    def rowslice(self, index):
        row_slices = OrderedDict((k, v[index]) for k, v in self._data.items())
    ...
    def _append_columns(self, other_df):
        ...
        data = self._data.copy()
        for column, array in other_df._data.items():
            data[column] = array
        new_df = type(self)(data)
        return new_df
    ...
    def _check_data_format(self, dct):
        if not all(type(v) == np.ndarray for v in dct.values()):
            raise RuntimeWarning('Not all values are NumPy array. ')

        unique_ndims = np.unique([v.ndim for v in dct.values()])
        ...
        lengths = np.unique([len(v) for v in dct.values()])
        if len(lengths) > 1:
...
class SeamlessMixin(object):
    ...
    def group_reduce(self, group_ids, func):
        ...
        for group, group_ix in group_ixs.items():
    ...
    def _group_apply(self, group_ixs, func):
        ...
        applied_arr = None
        for group_ix in group_ixs.values():
            group_ddf = self.rowslice(group_ix)
...
class HorizontalFormatter(object):
    ...
    def _get_repr_item_generator(self, df):
        ...
        for j, (column, value) in enumerate(df.data.items()):
